...
function computeSettlement(historyOrders, persons) {
  const netBalance = {};
  const allItems = {};
  const rounds = [];
  ...
  persons.forEach((p) => {
    const entries = Object.entries(p.items).filter(([_, o]) => o.qty > 0);
...
function SplitwiseModal({ open, onClose, persons, sessionCode }) {
  ...
  const [loading, setLoading] = useState(false);
  const [computed, setComputed] = useState({ personTotals: [], groupTotal: 0, settlements: [], roundDetails: [], hasActive: false });
  useEffect(() => {
